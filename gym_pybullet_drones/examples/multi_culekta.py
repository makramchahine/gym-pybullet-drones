...
DEFAULT_OUTPUT_FOLDER = f'train_m1_small_{DEFAULT_RECORD_FREQ_HZ}' # 'train_v11_fast_init_pp_60hz'
DEFAULT_COLAB = False
...
def run(
    ...
    Theta = [Theta_base + i * np.pi * 0.875 for i in range(num_drones)]
    ...
    TARGET_POS = np.zeros((num_drones, ACTIVE_WP + INIT_WP, 3))
    TARGET_ATT = np.zeros((num_drones, ACTIVE_WP + INIT_WP, 3))
    ...
    for d in range(num_drones):
        for i in range(ACTIVE_WP + INIT_WP):
            if i < INIT_WP:
                adj_R = R * DEVIATION_A
                adj_Delta_Theta = Delta_Theta0[d]

                TARGET_POS[i, :] = (
                    adj_R * np.cos((sign * i / ACTIVE_WP) * (2 * np.pi) + Theta[d]), 
                    adj_R * np.sin((sign * i / ACTIVE_WP) * (2 * np.pi) + Theta[d]),
                    0
                )
                TARGET_ATT[i, :] = 0, 0, (sign * i / ACTIVE_WP) * (2 * np.pi) + Theta0[d] + adj_Delta_Theta
            else:
                adj_i = i - INIT_WP
                ...
                if adj_i < RECOVERY_ANGLE_WP:
                    adj_Delta_Theta = Delta_Theta0[d] * (((RECOVERY_ANGLE_WP - adj_i) / RECOVERY_ANGLE_WP))
                ...
                if adj_i < SWITCH_WP:
                    ...
                    TARGET_ATT[d, i, :] = 0, 0, (sign * i / ACTIVE_WP) * (2 * np.pi) + Theta0[d] + adj_Delta_Theta
                else:
                    TARGET_POS[d, i, :] = (
                        adj_R * np.cos((sign * (2 * SWITCH_WP - adj_i + INIT_WP) / ACTIVE_WP) * (2 * np.pi) + Theta[d]),
                        adj_R * np.sin((sign * (2 * SWITCH_WP - adj_i + INIT_WP) / ACTIVE_WP) * (2 * np.pi) + Theta[d]),
                        0
                    )
                    TARGET_ATT[d, i, :] = 0, 0, (sign * (2 * SWITCH_WP - adj_i) / ACTIVE_WP) * (2 * np.pi) + Theta0[d] + adj_Delta_Theta
    ...
    LABEL = [[] for i in range(num_drones)]
        ...
        if i % CTRL_EVERY_N_STEPS == 0:
            ...
            for j in range(num_drones):
                ...
                action[str(j)], _, _ = ctrl[j].computeControlFromState(
                    ...
                    state=state,
                    target_pos=np.hstack([TARGET_POS[j, wp_counters[j], 0:2], INIT_XYZS[j, 2]]),
                    target_rpy=np.hstack([INIT_RPYS[j, 0:2], TARGET_ATT[j, wp_counters[j], 2]]),
                    )
    ...
    with open(sim_dir + "/values.csv", 'wb') as out_file:
        np.savetxt(out_file, np.array(LABEL).T, delimiter=",")
    ...
    if debug_plots:
        for d in range(num_drones):
            data = np.genfromtxt(sim_dir + f"/log_{d}.csv", delimiter=",", skip_header=2)
            
            fig, ax = plt.subplots()
            ax.plot(data[:, 1], data[:, 2])
            theta = np.linspace(0, 2 * np.pi, 100)
            ax.plot(R * np.cos(theta), R * np.sin(theta))
            ax.plot(TARGET_POS[d, :, 0], TARGET_POS[d, :, 1])
