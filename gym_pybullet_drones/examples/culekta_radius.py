...
DEFAULT_OUTPUT_FOLDER = f'train_m2_{DEFAULT_RECORD_FREQ_HZ}' # 'train_v11_fast_init_pp_60hz'
DEFAULT_COLAB = False
...
def run(
    ...
    DEVIATION_A0 = random.uniform(0.5, 1.5) # initial deviation from the circular trajectory
    DEVIATION_A1 = random.uniform(0.5, 1.5) # initial deviation from the circular trajectory
    DEVIATION_A2 = random.uniform(0.5, 1.5) # initial deviation from the circular trajectory
    ...
    for i in range(ACTIVE_WP + INIT_WP):
        if i < INIT_WP:
            ...
            adj_i = i - INIT_WP
            # First recovery within the first half of the period
            if adj_i < RECOVERY_WP / 2:
                percent_complete = (adj_i / (RECOVERY_WP / 2))
                adj_R = R * (DEVIATION_A0 + percent_complete * (DEVIATION_A1 - DEVIATION_A0))
            elif adj_i < RECOVERY_WP and adj_i >= RECOVERY_WP / 2:
                adj_R = R * DEVIATION_A1
            elif adj_i < RECOVERY_WP + (ACTIVE_WP - RECOVERY_WP) / 2 and adj_i >= RECOVERY_WP:
                percent_complete = ((adj_i - RECOVERY_WP) / ((ACTIVE_WP - RECOVERY_WP) / 2))
                adj_R = R * (DEVIATION_A1 + percent_complete * (DEVIATION_A2 - DEVIATION_A1))
            else:
                adj_R = R * DEVIATION_A2
        ...
        if i % REC_EVERY_N_STEPS == 0 and i>env.SIM_FREQ and i>=INIT_STEPS:
            ...
            for j in range(num_drones):
                ...
                if wp_counters[j] < SWITCH_WP + INIT_WP:
                    ...
                    LABEL_D.append(-sign)
                if wp_counters[j] < RECOVERY_WP + INIT_WP:
                    LABEL_R.append(DEVIATION_A1)
                else:
                    LABEL_R.append(DEVIATION_A2)
    ...
    with open(sim_dir + "/valuesD.csv", 'wb') as out_file:
        np.savetxt(out_file, LABEL_D, delimiter=",")
    with open(sim_dir + "/valuesR.csv", 'wb') as out_file:
        np.savetxt(out_file, LABEL_R, delimiter=",")
