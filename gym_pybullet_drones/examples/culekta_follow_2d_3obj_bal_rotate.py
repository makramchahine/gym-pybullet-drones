...
def run(
    ...
    def add_random_targets(target_colors, target_locations):
        ...
        return add_target(sampled_order, target_colors, target_locations)
    ...
    INIT_XYZS = np.array([[*convert_to_global(rel_pos, Theta), H] for rel_pos, H in zip(rel_drone_locs, [H_top, H_bottom])])
    ...
    while len(RAND_SWITCH_TIMES) < NUM_SWITCHES: #np.abs(np.array(speeds)).max() > 1e-6:
        speeds, aligned = step_with_distance_and_angle(angle_counter, INIT_THETA, FINAL_THETA, DELTA_THETA, TARGET_LOCATIONS, target_index)
        angle_counter += 1
        ...
        if switch and (np.array(angle_counter) % 30 == 0) and np.all(aligned) and random.random() < 0.8:
            target_colors, target_locations = add_random_targets(target_colors, target_locations)
...
if __name__ == "__main__":
    ...
    LCR_env_colors = PERMUTATIONS_COLORS * num_initializations * 3
    ...
    assert len(total_list) == num_initializations * 6 * 3, f"len(total_list): {len(total_list)}"
    ...
    import joblib
    joblib.Parallel(n_jobs=18)(joblib.delayed(run_func)(d) for d in tqdm(total_list))
